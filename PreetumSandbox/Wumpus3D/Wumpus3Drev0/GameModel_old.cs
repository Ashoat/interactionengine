...
namespace Wumpus3Drev0
{
    class GameModel_old
    {
        ...
        public GameModel_old(Model model, ModelEffect effect, Terrain terrain, GraphicsDevice device)
        {
            this.model = model;
        ...
        {
            this.target = tar;
            this.Rotation = HelperClass.GetAngle((this.Position2 - this.target));
        }
        public void UpdateMove()
        {
            this.Position2 += (this.target - this.Position2) * (this.Position2 - this.target).Length() * speed;
        }
